...
(function (document, Granite, $) {
	...
	function replicateToPublish(activator) {
		...
		function checkReferences(json) {
			...
			for (var a = 0; a < assets.length; a++) {
				...
				if ((!asset.published || asset.outdated) && params.indexOf(asset.path) === -1 && ["asset", "tag", "s7set", "collection", "contentfragmentmodel"].indexOf(asset.type) !== -1) {
					hasReferences = true;
				} else {
	...
	/* Function to trigger pop up based on Effective Date - Starts */
	function replicateToPublishWithEffectiveDateChk(activator) {
		var $collection = $(".cq-damadmin-admin-childpages.foundation-collection");
		var items = $collection.find(".foundation-selections-item");
		var effectiveFlagCt = 0;
		var beforeEffectiveDateCt = 0;
		for (var i = 0; i < items.length; i++) {
			var assetPath = $(items[i]).data("foundation-collection-item-id");
			var domain = window.location.origin;
			var metadataJSON = "/jcr:content/metadata.tidy.json";
			var assetMetadataEndpoint = domain + assetPath + metadataJSON
			console.log("Asset metadata endpoint=" + assetMetadataEndpoint);

			$.ajax({
				url: assetMetadataEndpoint,
				type: "get",
				async: false,
				success: function (data) {
					var json = JSON.stringify(data);
					var effectiveDateFlag = JSON.parse(json)["effectiveDateFlag"];
					if (effectiveDateFlag == "yes") {
						effectiveFlagCt = effectiveFlagCt + 1;
						var effectiveDate = JSON.parse(json)["effectiveDate"];
						var effectiveDateObj = new Date(effectiveDate);
						var currentDateObj = new Date();
						console.log("Effective date=" + effectiveDateObj + " and Current Date=" + currentDateObj);
						if (currentDateObj < effectiveDateObj) {
							console.log("Trying to publish before Effective Date");
							beforeEffectiveDateCt = beforeEffectiveDateCt + 1;
						}
					}
				},
				error: function () {
					console.log("Error accessing Asset Metadata");
				}
			});
		...
		intro.text(Granite.I18n.get("Selected Asset(s) Effective publish date is yet to arrive"));
		if (effectiveFlagCt > 0 && beforeEffectiveDateCt > 0) {
			ui.prompt("Asset Quick Publish", message.html(), "notice", [{
				...
				primary: true,
				handler: function () {
					console.log("Publish action flow");
	...
	/* Function to trigger pop up based on Effective Date - Ends */
	...
	$(document).on("click", rel + "-activator", function (e) {
		...
		if ($(this).hasClass("s7on")) {
			...
			/* Custom code for pop up - Starts */
			replicateToPublishWithEffectiveDateChk(activator);
			/* Custom code for pop up - Ends */
		}
	});
